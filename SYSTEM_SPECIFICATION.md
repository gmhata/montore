...
| # | プロンプト名 | 用途 | モデル | API/場所 |
|---|-------------|------|--------|----------|
| 1 | 患者シミュレーション | リアルタイム会話で患者役を演じる | gpt-4o-realtime-preview | `public/practice.js` buildInstructions() |
| 2 | 評価用システム | 会話ログを採点 | gpt-4o-mini | `/api/sessions/:id/finish` |
| 3 | 評価用ユーザー | 会話ログを入力 | gpt-4o-mini | `/api/sessions/:id/finish` |
| 4 | AI分析（管理者用） | 全学生の学習データ分析 | gpt-4o-mini | `/api/admin/ai-analysis` |
| 5 | AI分析（学生用） | 個人の学習データ分析 | gpt-4o-mini | `/api/users/:uid/ai-analysis` |
| 6 | 患者プロフィール生成（管理者用） | 症状から患者を自動生成 | gpt-4o | `/api/admin/patients/generate` |
| 7 | 患者プロフィール生成（学生用） | 症状から患者を自動生成（簡易版） | gpt-4o | `/api/generate-patient` |
